...
# Pure-core replay classification: exact duplicate, semantic duplicate, or conflict.
replace_once(
    "modules/crm-customer-enrichment/src/lifecycle.rs",
...
# Worker semantic idempotency and explicit reconciliation outcome.
worker = "crates/crm-customer-enrichment-worker-composition/src/lib.rs"
...
insert_before = '''fn provider_response_class_to_wire(value: ProviderResponseClass) -> i32 {
'''
helpers = r'''fn response_semantic_input_hash(
...
fn response_received_usage(
    ...
    let mut matching = response.provider_usage_entries.iter().filter(|entry| {
        entry.kind == wire::ProviderUsageKind::ResponseReceived as i32
    });
...
'''
replace_once(worker, insert_before, helpers + insert_before)
replace_once(
    worker,
...
tests = "crates/crm-customer-enrichment-worker-composition/src/tests.rs"
replace_once(
    tests,
    "use super::*;\n",
...
# Fresh PostgreSQL process: new, exact duplicate, semantic duplicate, conflict.
process = "crates/crm-customer-enrichment-worker-composition/tests/postgres_worker_process.rs"
replace_once(
    process,
