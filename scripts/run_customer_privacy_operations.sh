...
apply_database_inputs() {
  local database="$1"
  for path in database/migrations/*up.sql; do
    admin_psql "$database" < "$path"
  done
...
docker run --detach --rm \
  ...
  --env "POSTGRES_DB=${OPS_SOURCE_DATABASE}" \
  --env POSTGRES_USER=postgres \
  --env POSTGRES_PASSWORD=postgres \
  "$OPS_POSTGRES_IMAGE" >/dev/null

for _ in $(seq 1 120); do
  if docker exec "$CONTAINER_NAME" pg_isready \
      --username postgres --dbname "$OPS_SOURCE_DATABASE" >/dev/null 2>&1; then
    break
  fi
  sleep 0.5
done
...
if [ "$(stat -c '%a' "$BACKUP_PATH")" != "600" ]; then
  echo "backup artifact permissions are not 0600" >&2
  exit 1
fi
...
RESTORE_TABLE_COUNT="$(admin_psql "$OPS_RESTORE_DATABASE" --tuples-only --no-align --command \
  "SELECT count(*) FROM information_schema.tables WHERE table_schema NOT IN ('pg_catalog', 'information_schema');" | tr -d '[:space:]')"
if [ -z "$SOURCE_TABLE_COUNT" ] || [ "$SOURCE_TABLE_COUNT" != "$RESTORE_TABLE_COUNT" ]; then
  echo "restored table inventory does not match source: ${SOURCE_TABLE_COUNT} != ${RESTORE_TABLE_COUNT}" >&2
  exit 1
fi
...
CRM_DATABASE_URL="$RESTORE_APP_URL" \
CRM_API_BEARER_TOKEN="$TOKEN" \
...
CRM_GRPC_BIND="127.0.0.1:${GRPC_PORT}" \
CRM_HTTP_BIND="127.0.0.1:${HTTP_PORT}" \
./target/debug/crm-api >"$API_LOG_PATH" 2>&1 &
API_PID=$!

for _ in $(seq 1 $((OPS_STARTUP_SLO_SECONDS * 4))); do
  if curl --fail --silent --show-error \
      "http://127.0.0.1:${HTTP_PORT}/readyz" >/dev/null 2>&1; then
    break
  fi
  if ! kill -0 "$API_PID" 2>/dev/null; then
    cat "$API_LOG_PATH" >&2
    echo "crm-api exited before restored readiness" >&2
    exit 1
  fi
  sleep 0.25
done
...
curl --fail --silent --show-error "http://127.0.0.1:${HTTP_PORT}/readyz" \
...
for _ in $(seq 1 "$OPS_PROBE_COUNT"); do
  if latency="$(curl --fail --silent --show-error --output /dev/null \
      --write-out '%{time_total}' "http://127.0.0.1:${HTTP_PORT}/readyz")"; then
...
for _ in $(seq 1 120); do
  if curl --fail --silent --show-error "http://127.0.0.1:${VITE_PORT}" >/dev/null 2>&1; then
    break
  fi
  if ! kill -0 "$VITE_PID" 2>/dev/null; then
    cat "$VITE_LOG_PATH" >&2
    echo "Vite exited before restored browser acceptance" >&2
    exit 1
  fi
  sleep 0.5
done
...
pnpm --filter @ultimate-crm/web exec playwright test \
  ...
  --timeout="$((OPS_BROWSER_TIMEOUT_SECONDS * 1000))"

curl --fail --silent --show-error "http://127.0.0.1:${HTTP_PORT}/metrics" \
  > "$METRICS_PATH"
if grep --fixed-strings --quiet "$TOKEN" "$METRICS_PATH"; then
  echo "metrics output contains the bearer token" >&2
  exit 1
fi
