...
replace_once(
    "Cargo.toml",
    '  "crates/crm-customer-enrichment-provider-registry",\n',
    '  "crates/crm-customer-enrichment-provider-registry",\n'
    '  "crates/crm-customer-enrichment-registry-http-transport",\n',
)

crate_root = ROOT / "crates/crm-customer-enrichment-registry-http-transport"
(crate_root / "src").mkdir(parents=True, exist_ok=True)

(crate_root / "Cargo.toml").write_text(
    '''[package]
name = "crm-customer-enrichment-registry-http-transport"
version = "0.1.0"
edition = "2024"
publish = false
...
crm-customer-enrichment-provider-registry = { path = "../crm-customer-enrichment-provider-registry" }
...
(crate_root / "src/lib.rs").write_text(r'''#![forbid(unsafe_code)]

//! Concrete bounded HTTP transport for the first Customer Enrichment provider coordinate.

use crm_customer_enrichment::{ProviderDispatchRequest, ProviderResponseClass, SanitizedProviderResponse};
...
impl ProviderTransportPort for RegistryHttpTransport {
    fn dispatch<'a>(
        ...
        Box::pin(async move {
            ...
            let timeout = effective_timeout(provider_request, self.clock.as_ref(), self.config.timeout)?;
            ...
            let response = self
                ...
                .header("idempotency-key", &provider_request.provider_idempotency_key)
                .timeout(timeout)
                .body(body)
                .send()
                .await
...
fn serialize_request(
    ...
    .map_err(|_| failure(ProviderTransportFailureClass::Terminal, "request_encoding_failed"))?;
...
fn exactly_one_secret(
    request: &ProviderTransportRequest,
) -> Result<&crm_customer_enrichment_provider_registry::ProviderSecretMaterial, ProviderTransportFailure>
{
...
fn parse_response(
    ...
    if parsed.schema_version != RESPONSE_SCHEMA_VERSION || parsed.replay_key != request.provider_idempotency_key {
        return Err(failure(
            ProviderTransportFailureClass::MappingConflict,
    ...
    if parsed.metered_units > MAXIMUM_METERED_UNITS
        || !valid_optional_text(parsed.provider_correlation_id.as_deref(), MAXIMUM_CORRELATION_BYTES)
        || !valid_optional_text(
        ...
        || parsed.provider_observed_at_unix_ms.is_some_and(|value| value < 0)
    {
        return Err(failure(
            ProviderTransportFailureClass::MappingConflict,
...
fn effective_timeout(
    ...
    let remaining = request.deadline_at_unix_ms.checked_sub(now_unix_ms).filter(|value| *value > 0).ok_or_else(|| {
        failure(
            ProviderTransportFailureClass::Retryable,
...
fn failure(class: ProviderTransportFailureClass, safe_code: &'static str) -> ProviderTransportFailure {
    ProviderTransportFailure::try_new(class, Some(safe_code.to_owned())).unwrap_or_else(|_| {
        unreachable!("static provider failure codes are canonical")
    })
}
...
mod tests {
    ...
    fn adapter(
        ...
        let secrets = StaticProviderSecretHandleResolver::try_new([ProviderSecretRegistration {
            ...
            material: ProviderSecretMaterial::try_new(
                b"super-secret-provider-token".to_vec(),
            )
            .unwrap(),
        }])
        .unwrap();
    ...
    fn configuration_requires_an_exact_allowlisted_endpoint() {
        assert!(RegistryHttpTransportConfig::try_new(
    ...
    async fn successful_http_dispatch_returns_only_sanitized_evidence() {
        ...
        assert!(response.canonical_response_digest.iter().any(|byte| *byte != 0));
...
replace_once(
    ...
    'crm-customer-enrichment-provider-registry = { path = "../crm-customer-enrichment-provider-registry" }\n',
    'crm-customer-enrichment-provider-registry = { path = "../crm-customer-enrichment-provider-registry" }\n'
    'crm-customer-enrichment-registry-http-transport = { path = "../crm-customer-enrichment-registry-http-transport" }\n',
)

registry_path = "crates/crm-application-runtime/src/customer_enrichment_provider_registry.rs"
replace_once(
    registry_path,
...
replace_once(
    ...
    'const NANOS_PER_SECOND: u64 = 1_000_000_000;\n'
    'const REGISTRY_HTTP_ENDPOINT_ENVIRONMENT: &str =\n'
    '    "CRM_CUSTOMER_ENRICHMENT_REGISTRY_HTTP_ENDPOINT";\n'
...
replace_once(
    ...
    insertion + '/// Process-environment secret source used by the default production assembly.\n',
)
replace_once(
    registry_path,
...
replace_once(
    ...
    '    SystemClock, bootstrap_export_selection_worker_access,\n'
    '    build_process_customer_enrichment_provider_transport_catalog,\n',
)
replace_once(
    "crates/crm-application-runtime/src/runtime.rs",
