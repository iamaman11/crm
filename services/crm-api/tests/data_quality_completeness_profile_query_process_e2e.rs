...
const TOKEN: &str = "data-quality-profile-query-token-0123456789abcdef0123456789abcdef";
const APPROVAL_KEY: &str = "data-quality-profile-query-approval-key-0123456789abcdef";
...
async fn crm_api_process_queries_profiles_with_exact_parent_visibility_and_tenant_isolation() {
    let Ok(database_url) = std::env::var("DATABASE_URL") else {
        eprintln!(
            "skipping completeness-profile query process acceptance because DATABASE_URL is absent"
        );
        return;
    };
    ...
    let admin = PgPool::connect(&admin_database_url)
        .await
        .expect("connect completeness-profile query fixture publisher");
    admin
    ...
    let tenant_a_profile = publish_profile(
        ...
        TENANT_A,
        rule_set_ref(&tenant_a_rule_set),
        "display_name.profile_query_a",
    )
    .await;
    let tenant_a_profile_ref = profile_ref(&tenant_a_profile);

    let disclosed_a = get_profile(
        &mut grpc,
    ...
    .await
    .expect("query tenant A completeness profile through production gateway");
    ...
    let tenant_b_profile = publish_profile(
        ...
        &publish_profile_definition,
        TENANT_B,
        rule_set_ref(&tenant_b_rule_set),
        "display_name.profile_query_b",
    )
    .await;
    let tenant_b_profile_ref = profile_ref(&tenant_b_profile);
    assert_ne!(
        tenant_a_profile_ref.completeness_profile_version_id,
    ...
    .await
    .expect_err(
        "tenant-authorized actor must not read another tenant's completeness-profile record",
    );
    assert_eq!(cross_tenant.code(), Code::NotFound);

    let unavailable = get_profile(
        &mut grpc,
        &get_profile_definition,
    ...
    assert_eq!(cross_tenant.message(), unavailable.message());

    send_sigint(&child).await;
    let status = child
        .wait()
        .await
        .expect("wait for completeness-profile query acceptance crm-api");
...
async fn publish_rule_set(
    ...
    let response = mutate(
            ...
            data_quality::PublishPartyRuleSetVersionRequest {
                definition: Some(data_quality::PartyRuleSetDefinition {
                    ...
                    rules: vec![data_quality::PartyQualityRule {
                        ...
                        title: format!("Profile query threshold for {tenant_id}"),
        ...
        tenant_id,
        &unique_id("data-quality-profile-query-rule-set"),
    )
    .await
    .expect("publish completeness-profile query Party rule-set");
...
async fn publish_profile(
    ...
    let response = mutate(
        ...
        tenant_id,
        &unique_id("data-quality-profile-query-profile"),
    )
    .await
    .expect("publish completeness-profile query profile");
    ...
    .expect("decode completeness-profile publication response")
}
...
async fn get_profile(
    ...
    data_quality::GetPartyCompletenessProfileVersionResponse::decode(
        response
            .output
            .expect("completeness-profile query output")
            .payload
            .as_slice(),
    )
    .map_err(|error| Status::internal(format!("decode completeness-profile query: {error}")))
}
...
fn assert_disclosed_profile(
    ...
    let version = response
        ...
        .as_ref()
        .expect("disclosed completeness-profile version");
    assert_eq!(
...
fn profile_ref(
    ...
    response
        ...
        .expect("published completeness-profile version ref")
}
...
fn spawn_crm_api(database_url: &str, http_addr: &str, grpc_addr: &str) -> Child {
    Command::new(env!("CARGO_BIN_EXE_crm-api"))
        ...
        .env(
            ...
            "data-quality-profile-query-cursor-key-0123456789abcdef",
        )
        ...
        .kill_on_drop(true)
        .spawn()
        .expect("spawn production crm-api process for completeness-profile query acceptance")
}
...
async fn wait_until_ready(client: &reqwest::Client, child: &mut Child, http_addr: &str) {
    ...
    loop {
        if let Some(status) = child
            .try_wait()
            .expect("poll completeness-profile query acceptance crm-api")
        {
            panic!("crm-api exited before completeness-profile query readiness: {status}");
        }
        ...
        assert!(
            ...
            "completeness-profile query crm-api readiness timed out"
        );
    ...
    loop {
        match ApplicationGatewayServiceClient::connect(format!("http://{grpc_addr}")).await {
            ...
            Err(error) => {
                assert!(
                    Instant::now() < deadline,
                    "completeness-profile query gRPC listener timed out: {error}"
                );
                sleep(Duration::from_millis(200)).await;
            }
        }
...
async fn send_sigint(child: &Child) {
    let pid = child
        .id()
        .expect("running completeness-profile query crm-api has a PID");
    let status = Command::new("kill")
        .arg("-INT")
        .arg(pid.to_string())
        .status()
        .await
        .expect("send SIGINT to completeness-profile query crm-api");
    assert!(status.success(), "kill -INT failed: {status}");
}

fn free_port() -> u16 {
    TcpListener::bind("127.0.0.1:0")
        .expect("bind ephemeral completeness-profile query port")
        .local_addr()
        .expect("read ephemeral completeness-profile query port")
        .port()
}
