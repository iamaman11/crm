...
async fn crm_api_process_persists_retryable_target_failure_without_advancing_checkpoint_and_recovers()
{
    ...
    let suffix = format!("{}-{}", std::process::id(), now_unix_nanos());
    ...
    install_party_retryable_failure_trigger(&admin, &target_party_id).await;
...
async fn install_party_retryable_failure_trigger(admin: &PgPool, party_id: &str) {
    assert!(
        party_id
            .chars()
            .all(|character| character.is_ascii_alphanumeric() || matches!(character, '-' | '_')),
        "generated target Party id must be safe for the test-only SQL trigger"
    );
