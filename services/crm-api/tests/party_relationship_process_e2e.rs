...
async fn crm_api_process_serves_governed_party_relationship_lifecycle_and_rebuildable_hierarchy() {
    ...
    let missing_status = mutate(
        ...
        create_relationship_payload(
            &create,
            &unique_id("relationship-missing-party"),
    ...
    assert_eq!(
        ...
        "party-relationships.party_relationship"
    );
    assert_eq!(created.affected_resources[0].resource_id, employment_id);
    ...
    assert_eq!(relationship_id(&employment), employment_id);
    assert_eq!(from_party_id(&employment), employer_id);
    assert_eq!(to_party_id(&employment), employee_id);
    assert_eq!(
        relationship_directionality(&employment),
        relationships::PartyRelationshipDirectionality::Directional
    );
    assert_eq!(relationship_type_code(&employment), "employment");
    assert_eq!(
        relationship_status(&employment),
        relationships::PartyRelationshipStatus::Active
    );
    assert_eq!(resource_version(&employment), 1);

    let after_employment = relationship_evidence_counts(&admin, TENANT_A).await;
    assert_eq!(after_employment.records, baseline.records + 1);
    assert_eq!(after_employment.events, baseline.events + 1);
    assert_eq!(after_employment.audits, baseline.audits + 1);
    assert_eq!(after_employment.idempotency, baseline.idempotency + 1);
    assert_eq!(after_employment.transactions, baseline.transactions + 1);
    ...
    let household_output = household_created
        .output
        .expect("reciprocal Party Relationship output");
    let household = decode_create_relationship(&household_output.payload);
    let expected_first = std::cmp::min(employer_id.as_str(), household_peer_id.as_str());
    let expected_second = std::cmp::max(employer_id.as_str(), household_peer_id.as_str());
    assert_eq!(from_party_id(&household), expected_first);
    assert_eq!(to_party_id(&household), expected_second);
    assert_eq!(
        relationship_directionality(&household),
        relationships::PartyRelationshipDirectionality::Reciprocal
    );
    assert_eq!(relationship_type_code(&household), "household");

    let after_household = relationship_evidence_counts(&admin, TENANT_A).await;
    assert_eq!(after_household.records, after_employment.records + 1);
    assert_eq!(after_household.events, after_employment.events + 1);
    assert_eq!(after_household.audits, after_employment.audits + 1);
    assert_eq!(after_household.idempotency, after_employment.idempotency + 1);
    assert_eq!(after_household.transactions, after_employment.transactions + 1);

    let household_filter = query(
        &mut grpc,
        &list,
    ...
    let household_filter = decode_list_relationships(household_filter);
    assert_eq!(household_filter.party_relationships.len(), 1);
    assert_eq!(
        relationship_id(&household_filter.party_relationships[0]),
        household_id
    );
    ...
    let second_page = query(
        &mut grpc,
        &list,
        list_relationships_payload(&list, 1, &first_page_token, None, None, None, None),
        TENANT_A,
        true,
    )
    .await
    ...
    .await
    .expect("deactivate directional Party Relationship");
    ...
    assert_eq!(after_update.records, after_household.records);
    assert_eq!(after_update.events, after_household.events + 1);
    assert_eq!(after_update.audits, after_household.audits + 1);
    assert_eq!(after_update.idempotency, after_household.idempotency + 1);
    assert_eq!(after_update.transactions, after_household.transactions + 1);
    ...
    .await
    .expect("get updated Party Relationship");
    let queried_after_update = decode_get_relationship(queried_after_update);
    assert_eq!(
        relationship_status(&queried_after_update),
        relationships::PartyRelationshipStatus::Inactive
    );
    ...
    let projected_before_rebuild = hierarchy_documents(&admin, TENANT_A).await;
    assert_eq!(projected_before_rebuild.len(), 4);
    let employment_edges = projected_before_rebuild
        .iter()
    ...
    assert!(
        employment_edges
            .iter()
            .all(|document| document.status == "inactive" && document.version == 2)
    );
    let household_edges = projected_before_rebuild
        .iter()
    ...
    let traversal = traverse_projected_hierarchy(
        &projected_before_rebuild,
        from_party_id(&household),
        1,
        now_unix_nanos(),
    );
    assert_eq!(
    ...
    let inactive_traversal =
        traverse_projected_hierarchy(&projected_before_rebuild, &employer_id, 1, now_unix_nanos());
    assert_eq!(
        inactive_traversal,
        BTreeMap::from([(employer_id.clone(), 0)])
    );
    ...
    let projected_after_rebuild = hierarchy_documents(&admin, TENANT_A).await;
    assert_eq!(projected_after_rebuild, projected_before_rebuild);
