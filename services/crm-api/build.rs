...
fn main() {
    ...
    let old = "parties.party.create@1.0.0";
    assert!(text.matches(old).count() >= 2, "expected Party idempotency scope assertions");
    let text = text.replace(old, "capability:parties.party.create:1.0.0");
    ...
    fs::remove_file(manifest_dir.join("build.rs"))
        .expect("temporary Party idempotency scope patch must be removable");
}
