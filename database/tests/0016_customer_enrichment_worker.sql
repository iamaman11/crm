...
VALUES
  ...
  (
    'customer_enrichment.materialization.seed',
    '1.0.0',
    'crm.customer-enrichment',
    '0.1.0',
    'crm.customer_enrichment.test.MaterializationFixtureService',
    'SeedMaterializationDependencies',
    decode(repeat('e8', 32), 'hex'),
    decode(repeat('e9', 32), 'hex'),
    'medium', true, true, false, false, false, false, false,
    ARRAY['customer_enrichment.materialization.seed']::text[],
    ARRAY['personal', 'confidential']::text[]
  ),
  (
    ...
    decode(repeat('ea', 32), 'hex'),
    decode(repeat('eb', 32), 'hex'),
    'high', true, true, false, false, false, false, false,
    ARRAY['customer_enrichment.suggestions.materialize']::text[],
    ARRAY['personal', 'confidential']::text[]
  )
