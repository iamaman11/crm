...
This packet performs one conclusive safe reduction. `crm-customer-privacy-production` already owns the Customer Privacy control-query adapter, constructs its executor and validator, and is documented as the only supported owner production entry point. The generic runtime nevertheless depended directly on `crm-customer-privacy-query-adapter` to read the control-query inventory during process composition and to assemble bootstrap visibility resources.

Step 22C exposes both contracts through the owner production package and removes the redundant direct process-host edge.
...
1. `crm-customer-privacy-production` publicly re-exports `control_query_capability_definitions` from its legal-hold production surface.
2. The owner root publicly exposes `control_query_visibility_resources`; the already-supported legacy owner surface continues to expose case-query visibility and capability constants.
3. `customer_privacy_case_create_promotion.rs` and `bootstrap_visibility/registry.rs` consume those contracts only through `crm_customer_privacy_production`.
4. `crm-customer-privacy-query-adapter` is removed only from `crm-application-runtime/Cargo.toml`.
5. The production package keeps its existing query-adapter dependency and remains responsible for construction of the control-query adapter, validator and executor.
6. Existing owner-package re-exports are grouped canonically so the conservative public Rust surface remains exactly 5,377 rather than growing for syntax-only boundary exposure.
...
- process composition consumes `control_query_capability_definitions` through `crm_customer_privacy_production`;
...
Workspace compilation proves the visibility registry also resolves case and control visibility exclusively through the owner package after the direct adapter dependency is removed. Focused Rust tests and all affected process/PostgreSQL acceptance remain authoritative for behavior preservation.
