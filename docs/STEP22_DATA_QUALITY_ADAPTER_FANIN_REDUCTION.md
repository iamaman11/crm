...
The only direct Data Quality adapter source use found inside `crm-application-runtime` is the integration registration test importing capability-adapter constants. That test is moved to compare the application inventory against the existing owner composition mutation definition inventory; no new public owner API is added.
...
4. routes the registration test through the existing owner composition definition inventory rather than adapter constants;
...
- the registration test regains a direct Data Quality adapter reference;
