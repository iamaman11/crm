...
An absent aggregate has no row that PostgreSQL can protect with `FOR UPDATE`. Create planning therefore also requires transaction-scoped serialization over the tenant, owner module, record type and record id.
...
5. loads the owner-module record with `SELECT ... FOR UPDATE`;
...
- Competing updates and creates serialize on the aggregate identity and optimistic domain checks remain deterministic.
