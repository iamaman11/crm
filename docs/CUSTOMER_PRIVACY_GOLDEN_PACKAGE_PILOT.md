...
`Cargo.lock` was regenerated by permanent Rust Generated Sync. It adds the three workspace packages and replaces the five direct execution/composition edges from `crm-application-runtime` with the owner production edge while retaining the existing query visibility metadata edge; no third-party dependency version or feature resolution changes.
