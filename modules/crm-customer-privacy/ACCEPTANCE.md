...
- [x] Derive the exact cancellation subject lock-set from canonical binding plus pending rescope target, sort and deduplicate it, acquire shared subject locks before the case row, then repeat strict rehydration under `FOR SHARE`.
- [x] Fail retryably if the subject lock-set changes between discovery and locked recheck; never accept an unbound-to-bound TOCTOU transition and never reverse the subject-before-case lock order.
