...
The exact staged promotion contract is machine-readable at `contracts/customer-enrichment-production-promotion.json`. It is authoritative for the current 6-mutation + 6-query public runtime inventory, one activation-gated application worker and four individually non-runtime coordinates. Production coordinates must be promoted in that declared order unless the contract and its validator are changed together on one green exact head.
