...
- [x] Add non-runtime deterministic worker foundation for `customer_enrichment.request.dispatch@1.0.0` and `customer_enrichment.response.record@1.0.0`: standalone Personal mutation definitions, exact status/retry-generation expectations, Created/Queued/FailedRetryable dispatch semantics, fail-before-mutation sanitized receipt validation, immutable response-receipt identity and exact request binding with focused stale/invalid-evidence proof. Production registration remains blocked on an infrastructure-owned exact adapter registry and crash-safe atomic persistence/orchestration.
