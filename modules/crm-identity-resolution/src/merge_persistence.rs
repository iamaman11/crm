use crate::merge::{
    DisplayNameSurvivorship, MergeActorReference, MergeLineageId, MergeLineageStatus,
    MergePartyKind, PartyMergeLineage, PartyMergeLineageSnapshot, SurvivorshipSource,
    UnmergeDecision, UnmergeReasonCode,
};
...
pub fn encode_party_merge_lineage_state(lineage: &PartyMergeLineage) -> Result<Vec<u8>, SdkError> {
    let bytes = serde_json::to_vec(&PartyMergeLineageStateV1::from(lineage.snapshot())).map_err(
        |error| {
...
pub fn decode_party_merge_lineage_state(bytes: &[u8]) -> Result<PartyMergeLineage, SdkError> {
    ...
    let snapshot = PartyMergeLineageSnapshot::try_from(state)?;
    let lineage = PartyMergeLineage::rehydrate(snapshot)
...
impl From<PartyMergeLineageSnapshot> for PartyMergeLineageStateV1 {
    fn from(value: PartyMergeLineageSnapshot) -> Self {
        Self {
...
impl TryFrom<PartyMergeLineageStateV1> for PartyMergeLineageSnapshot {
    type Error = SdkError;

    fn try_from(value: PartyMergeLineageStateV1) -> Result<Self, Self::Error> {
        let merge_id = canonical_merge_id(value.merge_id)?;
        let candidate_case_id = canonical_case_id(value.candidate_case_id)?;
        let left_party_ref = canonical_party_ref(value.left_party_id.clone())?;
        let right_party_ref = canonical_party_ref(value.right_party_id.clone())?;
        ...
        let survivor_party_ref = canonical_party_ref(value.survivor_party_id)?;
        let absorbed_party_ref = canonical_party_ref(value.absorbed_party_id)?;
        if !pair.contains(&survivor_party_ref)
            || !pair.contains(&absorbed_party_ref)
            || survivor_party_ref == absorbed_party_ref
        {
            return Err(persisted_error(
        ...
        let merge_actor_ref = canonical_actor_ref(value.merge_actor_ref)?;
        let display_name_survivorship = value.display_name_survivorship.try_into()?;
        let unmerge_decision = value.unmerge_decision.map(TryInto::try_into).transpose()?;
...
impl TryFrom<DisplayNameSurvivorshipStateV1> for DisplayNameSurvivorship {
    type Error = SdkError;

    fn try_from(value: DisplayNameSurvivorshipStateV1) -> Result<Self, Self::Error> {
        let domain = DisplayNameSurvivorship::try_new(
            value.chosen_source.into(),
            value.survivor_value.clone(),
            value.absorbed_value.clone(),
        )
        .map_err(|error| persisted_error(error.to_string()))?;
        if domain.survivor_value() != value.survivor_value
            || domain.absorbed_value() != value.absorbed_value
        {
            return Err(persisted_error(
...
impl From<UnmergeDecision> for UnmergeDecisionStateV1 {
    fn from(value: UnmergeDecision) -> Self {
        Self {
...
impl TryFrom<UnmergeDecisionStateV1> for UnmergeDecision {
    type Error = SdkError;

    fn try_from(value: UnmergeDecisionStateV1) -> Result<Self, Self::Error> {
        Ok(UnmergeDecision::from_persisted_parts(
            canonical_actor_ref(value.actor_ref)?,
            canonical_unmerge_reason(value.reason)?,
            value.survivor_pre_unmerge_version,
            value.absorbed_pre_unmerge_version,
            value.survivor_post_unmerge_version,
            value.absorbed_post_unmerge_version,
            value.occurred_at_unix_nanos,
        ))
    }
}
...
impl From<MergeLineageStatusState> for MergeLineageStatus {
    fn from(value: MergeLineageStatusState) -> Self {
        match value {
            MergeLineageStatusState::Active => Self::Active,
            MergeLineageStatusState::Unmerged => Self::Unmerged,
        }
    }
}
...
mod tests {
    use super::*;
    use crate::merge::{CreatePartyMergeLineage, UnmergePartyLineage};
