...
class ArchitectureDocumentationConsistencyTests(unittest.TestCase):
    ...
    def test_active_packet_is_machine_declared_and_generated(self) -> None:
        ...
        for document in self.authoritative_status_documents:
            self.assertIn("PR #235", document)
            self.assertIn("7a0cd34dc17085ecd1a8ee233171c0463d91ceba", document)
            self.assertIn("43d194231fbce1cee28c44e89726929e450f3d18", document)
            self.assertIn("17 of 17", document)
            self.assertIn("repository step 8", document.lower())
