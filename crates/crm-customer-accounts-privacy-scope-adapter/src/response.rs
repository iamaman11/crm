...
use crm_customer_accounts::{ACCOUNT_STATE_RETENTION_POLICY_ID, MODULE_ID};
use crm_customer_accounts_capability_adapter::RECORD_TYPE;
...
pub(crate) struct VerifiedAccountResource {
...
pub(crate) fn build_response(
    ...
    Ok(privacy::CustomerAccountsPrivacyScopeContributionResponse {
            ...
            resources: resources
                ...
                .map(|resource| privacy::PrivacyScopeResourceReference {
                    ...
                    evidence_class: privacy::PrivacyScopeEvidenceClass::RetainMinimizedEvidence
                        as i32,
                    retention_policy_id: ACCOUNT_STATE_RETENTION_POLICY_ID.to_owned(),
                })
                .collect(),
            page_evidence: Some(privacy::PrivacyScopeContributionPageEvidence {
                ...
                emitted_resource_count: u64::try_from(resources.len()).map_err(|_| {
                    stored_state_invalid(
                        "Customer Accounts scope emitted count does not fit in u64",
                    )
                })?,
                next_cursor,
...
fn page_digest(
    ...
    for resource in resources {
        ...
        append_frame(&mut hasher, ACCOUNT_STATE_RETENTION_POLICY_ID.as_bytes());
    }
