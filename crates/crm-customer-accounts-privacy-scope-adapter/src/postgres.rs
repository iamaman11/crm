use crate::contract::{
    MAXIMUM_SCAN_RECORDS_PER_PAGE, customer_accounts_privacy_scope_definition, validate_definition,
};
use crate::errors::{
...
use crate::response::{VerifiedAccountResource, build_response, typed_output};
...
use crm_customer_accounts::{
    ...
    ACCOUNT_STATE_SCHEMA_VERSION, MODULE_ID, account_state_descriptor_hash,
};
use crm_customer_accounts_capability_adapter::{RECORD_TYPE, account_from_snapshot};
...
#[derive(Clone)]
pub struct CustomerAccountsPrivacyScopeQueryAdapter {
    store: PostgresDataStore,
}

impl std::fmt::Debug for CustomerAccountsPrivacyScopeQueryAdapter {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("CustomerAccountsPrivacyScopeQueryAdapter")
            .field("store", &"PostgresDataStore")
            .finish()
    }
}

impl CustomerAccountsPrivacyScopeQueryAdapter {
    ...
    async fn execute_query(
        ...
        let page = read_account_page(&mut transaction, &validated).await?;
...
impl QueryExecutor for CustomerAccountsPrivacyScopeQueryAdapter {
    fn execute<'a>(
        &'a self,
...
struct AccountPage {
    resources: Vec<VerifiedAccountResource>,
...
async fn read_account_page(
    ...
    loop {
            ...
            return Ok(AccountPage {
                ...
                scanned_resource_count: u64::try_from(scanned).map_err(|_| {
                    stored_state_invalid(
                        "Customer Accounts privacy scope scan count does not fit in u64",
                    )
                })?,
        ...
        let fetch_limit = i64::try_from(scan_limit.saturating_add(1)).map_err(|_| {
            stored_state_invalid("Customer Accounts privacy scope SQL limit does not fit in i64")
        })?;
        let rows = sqlx::query(
            r#"
            SELECT
        ...
        for (index, row) in rows.into_iter().take(available).enumerate() {
            ...
            let resource = strict_account_resource(&request.canonical_party_id, &record_id, row)?;
...
fn strict_account_resource(
    canonical_party_id: &RecordId,
    record_id: &RecordId,
    row: sqlx::postgres::PgRow,
) -> Result<Option<VerifiedAccountResource>, SdkError> {
    ...
    let expected_descriptor_hash = account_state_descriptor_hash();
    if version <= 0
        || owner_module_id != MODULE_ID
        || stored_schema_id != ACCOUNT_STATE_SCHEMA_ID
        || stored_schema_version != ACCOUNT_STATE_SCHEMA_VERSION
        || stored_descriptor_hash.as_slice() != expected_descriptor_hash
        || stored_data_class != "personal"
        || stored_encoding != "json"
        || stored_maximum_size != ACCOUNT_STATE_MAXIMUM_BYTES as i64
        || stored_retention != ACCOUNT_STATE_RETENTION_POLICY_ID
    {
        return Err(stored_state_invalid(
            "persisted Account metadata does not match the canonical state contract",
        ));
    }
    ...
    let snapshot = RecordSnapshot {
        ...
        payload: TypedPayload {
            ...
            schema_id: configured(SchemaId::try_new(ACCOUNT_STATE_SCHEMA_ID))?,
            schema_version: configured(SchemaVersion::try_new(ACCOUNT_STATE_SCHEMA_VERSION))?,
            ...
            retention_policy_id: configured(RetentionPolicyId::try_new(
                ACCOUNT_STATE_RETENTION_POLICY_ID,
            ))?,
            bytes: payload_bytes,
        },
    };
    ...
    if !account
        .party_associations()
        .iter()
        .any(|association| association.party().as_str() == canonical_party_id.as_str())
    {
        return Ok(None);
    }
    let resource_version = u64::try_from(account.version())
        .map_err(|_| stored_state_invalid("persisted Account version must be positive"))?;
    Ok(Some(VerifiedAccountResource {
        record_id: record_id.clone(),
        resource_version,
    }))
}
