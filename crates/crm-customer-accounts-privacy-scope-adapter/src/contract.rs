...
use crm_customer_accounts::MODULE_ID;
use crm_module_sdk::{
...
pub const CAPABILITY_ID: &str = "customer_accounts.privacy.scope.contribute";
pub const CAPABILITY_VERSION: &str = "1.0.0";
pub const INPUT_SCHEMA_ID: &str =
    "crm.customer_privacy.v1.CustomerAccountsPrivacyScopeContributionRequest";
pub const OUTPUT_SCHEMA_ID: &str =
    "crm.customer_privacy.v1.CustomerAccountsPrivacyScopeContributionResponse";
...
pub fn customer_accounts_privacy_scope_definition() -> Result<CapabilityDefinition, SdkError> {
...
pub(crate) fn validate_definition(definition: &CapabilityDefinition) -> Result<(), SdkError> {
    let expected = customer_accounts_privacy_scope_definition()?;
    if definition != &expected {
        return Err(crate::errors::invalid_contract(
            "CUSTOMER_ACCOUNTS_PRIVACY_SCOPE_DEFINITION_MISMATCH",
            "The Customer Accounts privacy scope definition is invalid.",
        ));
    }
    Ok(())
}
