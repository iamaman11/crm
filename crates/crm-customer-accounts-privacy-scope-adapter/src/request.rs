...
pub(crate) fn validate_request_contract(request: &QueryRequest) -> Result<(), SdkError> {
    let definition = customer_accounts_privacy_scope_definition()?;
...
pub(crate) fn validate_wire_request(
    ...
    let request = privacy::CustomerAccountsPrivacyScopeContributionRequest::decode(bytes).map_err(
        |error| {
            invalid_contract_with_reference(
                "CUSTOMER_ACCOUNTS_PRIVACY_SCOPE_REQUEST_INVALID",
                "The Customer Accounts privacy scope request is invalid.",
                error.to_string(),
            )
        },
    )?;
...
fn cursor_invalid() -> SdkError {
    invalid_contract(
        ...
        "The Customer Accounts privacy scope cursor is invalid.",
    )
}
