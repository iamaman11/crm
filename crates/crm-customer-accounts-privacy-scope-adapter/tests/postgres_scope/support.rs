...
use crm_proto_contracts::{
    crm::{
        accounts::v1 as accounts, customer::v1 as customer, customer_privacy::v1 as privacy,
        ...
        .execute(
            ...
            capability_request(
                ...
                &parties::CreatePartyRequest {
                    ...
                    display_name: format!("Account Scope Subject {party_id}"),
                },
            ),
        )
        .await
...
pub(crate) async fn create_account(
    ...
    tenant: &str,
    account_id: &str,
    associations: &[(&str, accounts::AccountPartyRole)],
    seed: u8,
) {
    executor
        .execute(
            ...
            capability_request(
                ...
                &accounts::CreateAccountRequest {
                    ...
                    name: format!("Private Account {account_id}"),
                    party_associations: associations
                        .iter()
                        .map(|(party_id, role)| accounts::AccountPartyAssociation {
                            party_ref: Some(customer::PartyRef {
                                party_id: (*party_id).to_owned(),
                            }),
                            role: *role as i32,
        ...
        .await
        .expect("create authoritative Customer Account fixture");
}
...
pub(crate) fn scope_request(
    ...
    let wire = privacy::CustomerAccountsPrivacyScopeContributionRequest {
    ...
    QueryRequest {
        owner_module_id: ModuleId::try_new(crm_customer_accounts::MODULE_ID).unwrap(),
        ...
        input: TypedPayload {
            owner: ModuleId::try_new(crm_customer_accounts::MODULE_ID).unwrap(),
...
pub(crate) async fn corrupt_account_metadata(admin: &PgPool, account_id: &str) {
    let mut transaction = admin
        .begin()
        .await
        .expect("begin isolated Account metadata corruption transaction");
    sqlx::query(
        r#"
        SELECT
          set_config('app.tenant_id', $1, true),
    ...
    .bind(CREATE_ACCOUNT_CAPABILITY)
    .bind("1.0.0")
    .bind(format!("account-{account_id}-tx"))
    .execute(&mut *transaction)
    .await
    .expect("bind isolated Account metadata corruption context");
    sqlx::query(
        r#"
        UPDATE crm.records
    ...
    .bind(TENANT_A)
    .bind(crm_customer_accounts::MODULE_ID)
    .bind(crm_customer_accounts_capability_adapter::RECORD_TYPE)
    .bind(account_id)
    .execute(&mut *transaction)
    .await
    .expect("corrupt isolated Account metadata fixture");
    transaction
        .commit()
        .await
        .expect("commit isolated Account metadata corruption fixture");
}
