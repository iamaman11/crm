...
pub const INPUT_RETENTION_POLICY_ID: &str = "crm.consents.privacy.scope.request";
pub const OUTPUT_RETENTION_POLICY_ID: &str = "crm.consents.privacy.scope.response";
pub const DEFAULT_PAGE_SIZE: u32 = 64;
pub const MAXIMUM_PAGE_SIZE: u32 = 128;
pub const MAXIMUM_CURSOR_BYTES: usize = 1_024;

pub fn consents_privacy_scope_definition() -> Result<CapabilityDefinition, SdkError> {
...
pub(crate) fn validate_definition(definition: &CapabilityDefinition) -> Result<(), SdkError> {
    let expected = consents_privacy_scope_definition()?;
    if definition != &expected {
        return Err(crate::errors::invalid_contract(
            "CONSENTS_PRIVACY_SCOPE_DEFINITION_MISMATCH",
            "The Consents privacy scope definition is invalid.",
        ));
    }
    Ok(())
}
