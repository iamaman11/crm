use crate::{
    ...
    MATERIALIZE_SUGGESTIONS_CAPABILITY, MATERIALIZE_SUGGESTIONS_REQUEST_SCHEMA,
    MATERIALIZE_SUGGESTIONS_RESPONSE_SCHEMA, MODULE_ID, enrichment_request_from_snapshot,
...
use crm_customer_enrichment::{
    ...
    SUGGESTION_RECORD_TYPE, SUGGESTION_STATE_MAXIMUM_BYTES, SUGGESTION_STATE_SCHEMA_ID,
    Suggestion, SuggestionCandidateDraft, SuggestionLifecycleStatus, TargetField, TargetSnapshot,
...
/// Non-runtime atomic planner with exact immutable materialization dependencies.
#[derive(Debug, Clone)]
...
fn positive_u64(value: i64, field: &'static str) -> Result<u64, SdkError> {
    ...
    if value == 0 {
        return Err(SdkError::invalid_argument(field, "Value must be greater than zero"));
    }
    Ok(value)
}
