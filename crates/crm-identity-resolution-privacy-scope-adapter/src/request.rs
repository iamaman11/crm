...
impl ResourceFamily {
    pub(crate) fn token(self) -> &'static str {
        match self {
            Self::CandidateCase => "candidate",
            Self::MergeOperation => "merge",
        }
    }

    fn parse(value: &str) -> Result<Self, SdkError> {
        match value {
            "candidate" => Ok(Self::CandidateCase),
            "merge" => Ok(Self::MergeOperation),
...
pub(crate) fn validate_request_contract(request: &QueryRequest) -> Result<(), SdkError> {
    let definition = identity_resolution_privacy_scope_definition()?;
...
pub(crate) fn validate_wire_request(
    ...
    let request = privacy::IdentityResolutionPrivacyScopeContributionRequest::decode(bytes)
        .map_err(|error| {
            invalid_contract_with_reference(
                "IDENTITY_RESOLUTION_PRIVACY_SCOPE_REQUEST_INVALID",
                "The Identity Resolution privacy scope request is invalid.",
                error.to_string(),
            )
        })?;
...
fn decode_cursor(
    ...
    if family == ResourceFamily::CandidateCase && after_record_id.is_none() {
        return Err(cursor_invalid());
    }
...
fn cursor_digest(
    ...
    framed_digest(
        b"crm.identity-resolution.privacy.scope.request-cursor/v1",
        &[
...
fn cursor_invalid() -> SdkError {
    invalid_contract(
        ...
        "The Identity Resolution privacy scope cursor is invalid.",
    )
}
