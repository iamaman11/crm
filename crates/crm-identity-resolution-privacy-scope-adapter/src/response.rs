...
use crate::errors::{configured, stored_merge_state_invalid};
...
use crm_identity_resolution_capability_adapter::{MERGE_OPERATION_RECORD_TYPE, RECORD_TYPE};
use crm_module_sdk::{
...
pub(crate) struct VerifiedIdentityResolutionResource {
    pub family: ResourceFamily,
    pub record_id: RecordId,
    pub resource_version: u64,
}

impl VerifiedIdentityResolutionResource {
    fn resource_type(&self) -> &'static str {
        match self.family {
            ResourceFamily::CandidateCase => RECORD_TYPE,
            ResourceFamily::MergeOperation => MERGE_OPERATION_RECORD_TYPE,
        }
    }

    fn retention_policy_id(&self) -> &'static str {
        match self.family {
            ResourceFamily::CandidateCase => DUPLICATE_CANDIDATE_CASE_STATE_RETENTION_POLICY_ID,
            ResourceFamily::MergeOperation => MERGE_OPERATION_STATE_RETENTION_POLICY_ID,
        }
    }
}
...
pub(crate) fn build_response(
    ...
    resources: &[VerifiedIdentityResolutionResource],
    scanned_resource_count: u64,
    next_state: Option<&CursorState>,
) -> Result<privacy::IdentityResolutionPrivacyScopeContributionResponse, SdkError> {
    let next_cursor = match next_state {
        Some(state) => encode_cursor(
            request,
            request.page_number.checked_add(1).ok_or_else(|| {
                stored_merge_state_invalid("Identity Resolution scope page number overflowed")
            })?,
            state,
        )?,
        None => String::new(),
    ...
    let cursor_digest = framed_digest(
        b"crm.identity-resolution.privacy.scope.cursor-evidence/v1",
        &[
    ...
    Ok(
        privacy::IdentityResolutionPrivacyScopeContributionResponse {
            contribution: Some(privacy::PrivacyScopeContributionResponseEnvelope {
                owner_module_id: crm_identity_resolution::MODULE_ID.to_owned(),
                ...
                page_evidence: Some(privacy::PrivacyScopeContributionPageEvidence {
                    ...
                    emitted_resource_count: u64::try_from(resources.len()).map_err(|_| {
                        stored_merge_state_invalid(
                            "Identity Resolution scope emitted count does not fit in u64",
                        )
                    })?,
                    next_cursor,
...
fn page_digest(
    ...
    append_frame(
        &mut hasher,
        b"crm.identity-resolution.privacy.scope.page/v1",
    );
