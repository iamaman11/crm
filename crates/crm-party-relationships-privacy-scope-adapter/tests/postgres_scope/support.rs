        ...
        .execute(
            ...
            capability_request(
                ...
                &parties::CreatePartyRequest {
                    ...
                    display_name: format!("Party Relationship Scope Subject {party_id}"),
                },
            ),
        )
        .await
...
pub(crate) async fn create_party_relationship(
    executor: &Arc<dyn TransactionalCapabilityExecutor>,
    definition: &CapabilityDefinition,
    tenant: &str,
    ...
    executor
        .execute(
            ...
            capability_request(
                ...
                CREATE_PARTY_RELATIONSHIP_CAPABILITY,
                CREATE_PARTY_RELATIONSHIP_SCHEMA,
                tenant,
                &format!("party-relationship-{party_relationship_id}"),
                600_000_000 + i64::from(seed),
                seed,
                &party_relationships::CreatePartyRelationshipRequest {
...
pub(crate) async fn update_party_relationship(
    ...
    executor
        .execute(
            ...
            capability_request(
                crm_party_relationships::MODULE_ID,
                UPDATE_PARTY_RELATIONSHIP_CAPABILITY,
        ...
        .await
        .expect("update authoritative Party Relationship fixture");
}
...
pub(crate) fn scope_request(
    ...
    let wire = privacy::PartyRelationshipsPrivacyScopeContributionRequest {
    ...
    QueryRequest {
        owner_module_id: ModuleId::try_new(crm_party_relationships::MODULE_ID).unwrap(),
        ...
        input: TypedPayload {
            owner: ModuleId::try_new(crm_party_relationships::MODULE_ID).unwrap(),
...
pub(crate) async fn corrupt_party_relationship_metadata(admin: &PgPool, party_relationship_id: &str) {
    let mut transaction = admin
        .begin()
        .await
        .expect("begin isolated Party Relationship metadata corruption transaction");
    sqlx::query(
        r#"
        SELECT
          set_config('app.tenant_id', $1, true),
    ...
    .bind(TENANT_A)
    .bind("actor-a")
    .bind("request-corrupt-party-relationship")
    .bind(CREATE_PARTY_RELATIONSHIP_CAPABILITY)
    .bind("1.0.0")
    .bind(format!("party-relationship-{party_relationship_id}-tx"))
    .execute(&mut *transaction)
    .await
    .expect("bind isolated Party Relationship metadata corruption context");
    sqlx::query(
        r#"
        UPDATE crm.records
        SET schema_id = 'crm.party-relationships.party-relationship.state.invalid'
        WHERE tenant_id = $1
    ...
    .bind(TENANT_A)
    .bind(crm_party_relationships::MODULE_ID)
    .bind(crm_party_relationships_capability_adapter::RECORD_TYPE)
    .bind(party_relationship_id)
    .execute(&mut *transaction)
    .await
    .expect("corrupt isolated Party Relationship metadata fixture");
    transaction
        .commit()
        .await
        .expect("commit isolated Party Relationship metadata corruption fixture");
}
