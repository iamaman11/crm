...
use crm_party_relationships::{MODULE_ID, PARTY_RELATIONSHIP_STATE_RETENTION_POLICY_ID};
use crm_party_relationships_capability_adapter::RECORD_TYPE;
use crm_proto_contracts::crm::customer_privacy::v1 as privacy;
use sha2::{Digest, Sha256};

#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct VerifiedPartyRelationshipResource {
...
pub(crate) fn build_response(
    ...
    resources: &[VerifiedPartyRelationshipResource],
    scanned_resource_count: u64,
    next_after_record_id: Option<&RecordId>,
) -> Result<privacy::PartyRelationshipsPrivacyScopeContributionResponse, SdkError> {
    let next_cursor = match next_after_record_id {
        Some(after_record_id) => encode_cursor(
            request,
            request.page_number.checked_add(1).ok_or_else(|| {
                stored_state_invalid("Party Relationships scope page number overflowed")
            })?,
            after_record_id,
        )?,
        None => String::new(),
    ...
    let cursor_digest = framed_digest(
        b"crm.party-relationships.privacy.scope.cursor-evidence/v1",
        &[
    ...
    Ok(
        privacy::PartyRelationshipsPrivacyScopeContributionResponse {
                ...
                page_evidence: Some(privacy::PrivacyScopeContributionPageEvidence {
                    ...
                    emitted_resource_count: u64::try_from(resources.len()).map_err(|_| {
                        stored_state_invalid(
                            "Party Relationships scope emitted count does not fit in u64",
                        )
                    })?,
                    next_cursor,
                    terminal_complete: next_after_record_id.is_none(),
...
fn page_digest(
    ...
    append_frame(
        &mut hasher,
        b"crm.party-relationships.privacy.scope.page/v1",
    );
    ...
    for resource in resources {
        ...
        append_frame(
            &mut hasher,
            PARTY_RELATIONSHIP_STATE_RETENTION_POLICY_ID.as_bytes(),
        );
    }
