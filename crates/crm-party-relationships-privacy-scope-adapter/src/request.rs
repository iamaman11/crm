...
pub(crate) fn validate_request_contract(request: &QueryRequest) -> Result<(), SdkError> {
    let definition = party_relationships_privacy_scope_definition()?;
...
pub(crate) fn validate_wire_request(
    ...
    let request = privacy::PartyRelationshipsPrivacyScopeContributionRequest::decode(bytes)
        .map_err(|error| {
            invalid_contract_with_reference(
                "PARTY_RELATIONSHIPS_PRIVACY_SCOPE_REQUEST_INVALID",
                "The Party Relationships privacy scope request is invalid.",
                error.to_string(),
            )
        })?;
...
fn cursor_digest(
    ...
    framed_digest(
        b"crm.party-relationships.privacy.scope.request-cursor/v1",
        &[
...
fn cursor_invalid() -> SdkError {
    invalid_contract(
        ...
        "The Party Relationships privacy scope cursor is invalid.",
    )
}
