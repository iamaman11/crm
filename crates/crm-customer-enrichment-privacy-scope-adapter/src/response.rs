...
use crm_customer_enrichment::{
    ...
    LIFECYCLE_STATE_RETENTION_POLICY_ID, PROVIDER_RESPONSE_CONFLICT_RECORD_TYPE,
    PROVIDER_RESPONSE_CONFLICT_STATE_RETENTION_POLICY_ID, PROVIDER_RESPONSE_RECEIPT_RECORD_TYPE,
    PROVIDER_USAGE_ENTRY_RECORD_TYPE, PROVIDER_USAGE_ENTRY_STATE_RETENTION_POLICY_ID,
...
pub(crate) struct VerifiedCustomerEnrichmentResource {
    pub family: ResourceFamily,
    pub record_id: RecordId,
    pub resource_version: u64,
}

impl VerifiedCustomerEnrichmentResource {
    fn resource_type(&self) -> &'static str {
        match self.family {
            ResourceFamily::Request => ENRICHMENT_REQUEST_RECORD_TYPE,
            ResourceFamily::ResponseReceipt => PROVIDER_RESPONSE_RECEIPT_RECORD_TYPE,
            ResourceFamily::ResponseConflict => PROVIDER_RESPONSE_CONFLICT_RECORD_TYPE,
            ResourceFamily::Suggestion => SUGGESTION_RECORD_TYPE,
            ResourceFamily::ReviewDecision => REVIEW_DECISION_RECORD_TYPE,
            ResourceFamily::ApplicationAttempt => APPLICATION_ATTEMPT_RECORD_TYPE,
            ResourceFamily::ProviderUsageEntry => PROVIDER_USAGE_ENTRY_RECORD_TYPE,
        }
    }
    ...
    fn retention_policy_id(&self) -> &'static str {
        match self.family {
            ...
            ResourceFamily::ProviderUsageEntry => PROVIDER_USAGE_ENTRY_STATE_RETENTION_POLICY_ID,
            _ => LIFECYCLE_STATE_RETENTION_POLICY_ID,
        }
    }
}
...
pub(crate) fn build_response(
    ...
    resources: &[VerifiedCustomerEnrichmentResource],
    scanned_resource_count: u64,
    next_state: Option<&CursorState>,
) -> Result<privacy::CustomerEnrichmentPrivacyScopeContributionResponse, SdkError> {
    let next_cursor = match next_state {
        Some(state) => encode_cursor(
            request,
            request.page_number.checked_add(1).ok_or_else(|| {
                association_state_invalid("Customer Enrichment privacy page number overflowed")
            })?,
            state,
        )?,
        None => String::new(),
    ...
    let cursor_digest = framed_digest(
        b"crm.customer-enrichment.privacy.scope.cursor-evidence/v1",
        &[
...
fn page_digest(
    ...
    append_frame(
        &mut hasher,
        b"crm.customer-enrichment.privacy.scope.page/v1",
    );
