...
impl ResourceFamily {
    pub(crate) fn token(self) -> &'static str {
        match self {
            ...
            Self::ResponseReceipt => "receipt",
            Self::ResponseConflict => "conflict",
            Self::Suggestion => "suggestion",
            Self::ReviewDecision => "review",
            Self::ApplicationAttempt => "application",
            Self::ProviderUsageEntry => "usage",
        }
    }
    ...
    fn parse(value: &str) -> Result<Self, SdkError> {
        match value {
            ...
            "receipt" => Ok(Self::ResponseReceipt),
            "conflict" => Ok(Self::ResponseConflict),
            "suggestion" => Ok(Self::Suggestion),
            "review" => Ok(Self::ReviewDecision),
            "application" => Ok(Self::ApplicationAttempt),
            "usage" => Ok(Self::ProviderUsageEntry),
...
pub(crate) fn validate_request_contract(request: &QueryRequest) -> Result<(), SdkError> {
    let definition = customer_enrichment_privacy_scope_definition()?;
...
pub(crate) fn validate_wire_request(
    ...
    let request = privacy::CustomerEnrichmentPrivacyScopeContributionRequest::decode(bytes)
        .map_err(|error| {
            invalid_contract_with_reference(
                "CUSTOMER_ENRICHMENT_PRIVACY_SCOPE_REQUEST_INVALID",
                "The Customer Enrichment privacy scope request is invalid.",
                error.to_string(),
            )
        })?;
...
pub(crate) fn encode_cursor(
    ...
    Ok(format!(
        "{CURSOR_VERSION}.{next_page_number}.{}.{after}.{}",
        state.family.token(),
        encode_hex(&digest)
    ))
}
...
fn decode_cursor(
    ...
    if family == ResourceFamily::Request && after_record_id.is_none() {
        return Err(cursor_invalid());
    }
...
fn cursor_digest(
    ...
    framed_digest(
        b"crm.customer-enrichment.privacy.scope.request-cursor/v1",
        &[
