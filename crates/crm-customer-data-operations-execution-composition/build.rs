...
fn main() {
    ...
    let old_import = "CapabilityClient, CapabilityId, CapabilityInvocation, CapabilityOutcome, CapabilityVersion,\n    DataClass, ErrorCategory, IdempotencyKey, ModuleExecutionContext, ModuleId, PortFuture,\n    SdkError,";
    if lib.contains(old_import) {
        lib = lib.replace(
            old_import,
...
"#;
    ...
    let old_sink_import = "CapabilityId, CapabilityVersion, DataClass, ErrorCategory, IdempotencyKey,\n    ModuleExecutionContext, ModuleId, PortFuture, RecordRef, SchemaVersion, SdkError, TypedPayload,";
    if sink.contains(old_sink_import) {
        sink = sink.replace(
            old_sink_import,
...
"#;
    ...
    Ok(CapabilityRequest {
"#;
    if sink.contains(old_outcome) {
        sink = sink.replacen(old_outcome, new_outcome, 1);
    }
    assert!(
        sink.contains("BusinessTransactionId::try_new(outcome_identity)"),
        "outcome transaction identity patch was not applied"
    );
