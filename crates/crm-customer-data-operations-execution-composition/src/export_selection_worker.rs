...
/// One production source boundary supplies both governed Party LIST selection and exact Party GET
/// execution reads. The capability authorizer is reused only to construct the export-owned execution
/// sink; every actual side effect still performs its own live authorization immediately before the
/// transactional write.
pub trait PartyExportSelectionExecutionSource:
    PartyExportSelectionSource + PartyExportExecutionSource
{
    fn capability_authorizer(&self) -> Arc<dyn CapabilityAuthorizer>;
}

#[derive(Clone)]
struct ExecutionSourceBridge {
    source: Arc<dyn PartyExportSelectionExecutionSource>,
}

impl PartyExportExecutionSource for ExecutionSourceBridge {
    fn get<'a>(
        &'a self,
        request: PartyExportExecutionSourceRequest<'a>,
    ) -> PortFuture<'a, Result<PartyExportExecutionSourceResult, SdkError>> {
        PartyExportExecutionSource::get(self.source.as_ref(), request)
    }
}
...
pub struct PartyExportSelectionWorker {
    ...
    source: Arc<dyn PartyExportSelectionExecutionSource>,
    execution_worker: Arc<PartyExportExecutionWorker>,
    clock: Arc<dyn Clock>,
    actor_id: ActorId,
    page_size: u32,
...
impl fmt::Debug for PartyExportSelectionWorker {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            ...
            .field("source", &"dyn PartyExportSelectionExecutionSource")
...
impl PartyExportSelectionWorker {
    ...
    pub fn execution_worker(&self) -> &PartyExportExecutionWorker {
