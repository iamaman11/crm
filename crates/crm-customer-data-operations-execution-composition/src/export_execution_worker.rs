            ...
            'jobs: for record in page.records {
                ...
                let manifest = self
                    ...
                    .load_manifest(
                        &tenant_id,
                        &job,
                        &crm_customer_data_operations::PartyExportSelectionProgress::rehydrate(
                            job.job_id().clone(),
                            job.specification().scope().maximum_resources(),
                            selection.selected_resources().saturating_add(1),
                            None,
                            true,
                            i64::from(selection.selected_resources()).saturating_add(1),
                        )?,
                    )
                    .await?;
                if manifest.len()
                ...
                for item in &manifest {
                    ...
                    let source = self
                        ...
                        .get(PartyExportExecutionSourceRequest {
                            ...
                            party_id: &RecordId::try_new(item.party_id().as_str())
                                .map_err(configuration_error)?,
...
async fn load_outcome_prefix(
    reader: &PostgresPartyExportExecutionReader,
    tenant_id: &TenantId,
    job_id: &ExportJobId,
    checkpoint: u32,
