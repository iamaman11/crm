...
fn main() {
    ...
    let text = fs::read_to_string(&test).expect("foundation test must be readable");
    let old = r#"        )
        .await
        .expect("commit evidenced file artifact capability transaction");
...
"#;
    ...
    assert!(!result.replayed);
"#;
    assert!(text.contains(old), "atomic artifact diagnostic anchor missing");
    fs::write(&test, text.replacen(old, new, 1)).expect("foundation test must be writable");

    run(repo, "cargo", &["fmt", "--all"]);
    fs::remove_file(manifest_dir.join("build.rs"))
        .expect("temporary atomic artifact diagnostic hook must be removable");
    ...
    let commit_status = Command::new("git")
        .args([
            "commit",
            "-m",
            "test(files): capture atomic artifact capability failure diagnostics",
        ])
        .current_dir(repo)
        .status()
