...
fn replace_all(path: &Path, old: &str, new: &str) {
...
fn main() {
    ...
    replace_all(
        &test,
        "CapabilityId::try_new(\"test.file.create\")",
        "CapabilityId::try_new(\"test.record.mutate\")",
    );
    replace_all(
        &test,
    ...
    fs::remove_file(manifest_dir.join("build.rs"))
        .expect("temporary capability registry fixture patch must be removable");
    ...
    let commit_status = Command::new("git")
        .args([
            "commit",
            "-m",
            "fix(files): use registered capability fixture in atomic evidence acceptance",
        ])
        .current_dir(repo)
        .status()
