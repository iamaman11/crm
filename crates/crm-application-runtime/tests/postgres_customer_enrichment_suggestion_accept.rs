...
use crm_customer_enrichment_review_adapter::{
    ACCEPT_SUGGESTION_CAPABILITY, accept_suggestion_capability_definition,
};
...
async fn production_suggestion_acceptance_requires_approval_and_is_replay_safe() {
    let Ok(database_url) = std::env::var("DATABASE_URL") else {
        eprintln!("skipping production suggestion acceptance because DATABASE_URL is absent");
        return;
    };
    ...
    let accept_definition =
        accept_suggestion_capability_definition().expect("valid acceptance definition");
    let get_definition =
    ...
    assert!(composition.mutation_definitions().iter().any(|candidate| {
        candidate.capability_id.as_str() == ACCEPT_SUGGESTION_CAPABILITY
    ...
    token_store
        .issue(
            ...
            AccessTokenGrant {
                ...
                authentication_id: "suggestion-production-accept-session".to_owned(),
                expires_at_unix_nanos: NOW + 10_000_000_000_000,
            },
        )
        .expect("issue production acceptance token");
    ...
    let approval_missing = execute_mutation(
        &mutation_http,
        &accept_definition,
        &accept_request(&suggestion, 7, None),
        TENANT,
        "suggestion-production-accept-approval-missing",
    )
    .await;
    assert_eq!(approval_missing.status, StatusCode::CONFLICT);
    assert_mutation_error_code(
        approval_missing.body,
    ...
    let policy_denied = execute_mutation(
        &mutation_http,
        &accept_definition,
        &accept_request(&suggestion, 7, Some("approval-production-1")),
        TENANT,
        "suggestion-production-accept-policy-denied",
    )
    .await;
    ...
    let hidden_party = execute_mutation(
        ...
        &accept_request(&suggestion, 7, Some("approval-production-1")),
        TENANT,
        "suggestion-production-accept-hidden-party",
    )
    .await;
    ...
    let stale = execute_mutation(
        &mutation_http,
        &accept_definition,
        &accept_request(&suggestion, 6, Some("approval-production-1")),
        TENANT,
        "suggestion-production-accept-stale",
    )
    .await;
    ...
    let cross_tenant = execute_mutation(
        ...
        &accept_request(&suggestion, 7, Some("approval-production-1")),
        OTHER_TENANT,
        "suggestion-production-accept-cross-tenant",
    )
    .await;
    ...
    for (status, key) in [
        ("suspended", "suggestion-production-accept-suspended"),
        ("uninstalling", "suggestion-production-accept-uninstalling"),
    ] {
    ...
    let success = execute_mutation(
        ...
        &accept_request(&suggestion, 7, Some("approval-production-1")),
        TENANT,
        "suggestion-production-accept-success",
    )
    .await;
    assert_eq!(success.status, StatusCode::OK);
    let response = wire::AcceptSuggestionResponse::decode(
        success_mutation_payload(success.body).bytes.as_slice(),
    )
    .expect("decode production acceptance response");
    ...
    assert_eq!(review.safe_reason_code, "reviewed_accepted");
    assert_eq!(
        review.approval_evidence_reference.as_deref(),
        Some("approval-production-1")
    );
    assert_eq!(
        response
            .suggestion
    ...
    .expect("decode accepted suggestion query response");
    assert_eq!(
        read_after_review
            .suggestion
    ...
    let latest_review = read_after_review
        .latest_review_decision
        .expect("latest acceptance");
    assert_eq!(
        latest_review.kind,
        wire::SuggestionReviewDecisionKind::Accepted as i32
    );
    assert_eq!(
        latest_review.approval_evidence_reference.as_deref(),
        Some("approval-production-1")
    );
    assert!(read_after_review.latest_application_attempt.is_none());
    assert_eq!(evidence_counts(&admin).await, after_success);

    authorization_store
        .revoke(
            &accept_authorization.tenant_id,
            &accept_authorization.actor_id,
            &accept_authorization.policy_id,
        )
        .expect("revoke top-level acceptance authorization");
    let top_level_denied = execute_mutation(
        &mutation_http,
        &accept_definition,
        &accept_request(&suggestion, 7, Some("approval-production-1")),
        TENANT,
        "suggestion-production-accept-top-level-denied",
    )
    .await;
...
fn accept_request(
    ...
    approval_evidence_reference: Option<&str>,
    ...
    wire::AcceptSuggestionRequest {
        ...
        approval_evidence_reference: approval_evidence_reference.map(str::to_owned),
        review_expires_at_unix_ms: Some(NOW / 1_000_000 + 50_000),
    }
}
