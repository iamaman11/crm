...
use crm_identity_resolution::{
    ...
    DUPLICATE_CANDIDATE_CASE_STATE_SCHEMA_VERSION, DecisionReasonCode, DecideDuplicateCandidateCase,
    DuplicateCandidateCase, DuplicateCandidateCaseId, DuplicateCandidateCaseStatus,
    EvidenceReference, MatchEvidenceSnapshot, MatchSignal, MatcherProfileCode, PartyReference,
    RefreshDuplicateCandidateEvidence, SignalKindCode, SignalSourceCode,
    CreateDuplicateCandidateCase, decode_duplicate_candidate_case_state,
...
pub fn duplicate_candidate_case_to_wire(candidate: &DuplicateCandidateCase) -> wire::DuplicateCandidateCase {
...
fn evidence_from_wire(
    ...
    let value = value.ok_or_else(|| SdkError::invalid_argument(field, "match evidence is required"))?;
...
fn case_id_from_ref(
    ...
    let value = value.ok_or_else(|| SdkError::invalid_argument(field, "candidate case ref is required"))?;
