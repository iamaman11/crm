...
use crm_contact_points::MODULE_ID;
use crm_module_sdk::{
...
pub const CAPABILITY_ID: &str = "contact_points.privacy.scope.contribute";
pub const CAPABILITY_VERSION: &str = "1.0.0";
pub const INPUT_SCHEMA_ID: &str =
    "crm.customer_privacy.v1.ContactPointsPrivacyScopeContributionRequest";
pub const OUTPUT_SCHEMA_ID: &str =
    "crm.customer_privacy.v1.ContactPointsPrivacyScopeContributionResponse";
...
pub const INPUT_RETENTION_POLICY_ID: &str = "crm.contact-points.privacy.scope.request";
pub const OUTPUT_RETENTION_POLICY_ID: &str = "crm.contact-points.privacy.scope.response";
...
pub fn contact_points_privacy_scope_definition() -> Result<CapabilityDefinition, SdkError> {
...
pub(crate) fn validate_definition(definition: &CapabilityDefinition) -> Result<(), SdkError> {
    let expected = contact_points_privacy_scope_definition()?;
    if definition != &expected {
        return Err(crate::errors::invalid_contract(
            "CONTACT_POINTS_PRIVACY_SCOPE_DEFINITION_MISMATCH",
            "The Contact Points privacy scope definition is invalid.",
        ));
    }
    Ok(())
}
