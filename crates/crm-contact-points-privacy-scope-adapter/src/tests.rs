use crate::contract::{
    ...
    contact_points_privacy_scope_definition, module_id, output_descriptor_hash, schema_id,
    schema_version,
};
use crate::errors::configured;
use crate::request::{encode_cursor, validate_request_contract, validate_wire_request};
use crate::response::{VerifiedContactPointResource, build_response};
...
fn query_request(message: &privacy::ContactPointsPrivacyScopeContributionRequest) -> QueryRequest {
...
#[test]
fn request_contract_preserves_contact_points_integrity_errors() {
...
fn cursor_round_trip_is_bound_to_lineage_and_page_size() {
    ...
    let cursor = encode_cursor(
        &validated,
        2,
        &RecordId::try_new("contact-point-001").unwrap(),
    )
    .unwrap();
    ...
    .unwrap_err();
    assert_invalid_argument(rebound, "CONTACT_POINTS_PRIVACY_SCOPE_CURSOR_INVALID");
}

#[test]
fn common_lineage_failures_keep_contact_points_codes() {
...
fn response_is_reference_only_deterministic_and_supports_sparse_progress() {
    ...
    .unwrap();
    let resources = vec![VerifiedContactPointResource {
        record_id: RecordId::try_new("contact-point-001").unwrap(),
        resource_version: 3,
    }];
    let next_after = RecordId::try_new("contact-point-127").unwrap();
    ...
    assert_eq!(envelope.resources[0].resource_id, "contact-point-001");
    assert_eq!(
...
fn malformed_wire_request_is_rejected_before_owner_reads() {
    ...
    assert_invalid_argument(error, "CONTACT_POINTS_PRIVACY_SCOPE_REQUEST_INVALID");
}
