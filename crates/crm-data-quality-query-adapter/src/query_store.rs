impl DataQualityQueryAdapter {
    ...
    async fn visible_or(
        ...
        let visibility = self
            .data_quality_visibility(request, &snapshot.reference)
            .await?;
        ...
        loop {
            ...
            for snapshot in &page.records {
                ...
                let visibility = self
                    ...
                    .await?;
                if visibility.resource_visible {
                    output.push(finding_to_wire_with_visibility(
                        &finding,
                        snapshot.version,
                        &visibility,
                    ));
    ...
    async fn has_more_visible_finding(
        &self,
        request: &QueryRequest,
        mut after: Option<RecordQueryContinuation>,
        filter: &FindingFilter,
