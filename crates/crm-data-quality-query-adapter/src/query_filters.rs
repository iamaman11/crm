fn validated_status_filter(value: Option<i32>) -> Result<Option<PartyFindingStatus>, SdkError> {
    value.map(|value| match wire::DataQualityFindingStatus::try_from(value) {
        Ok(wire::DataQualityFindingStatus::Open) => Ok(PartyFindingStatus::Open),
        Ok(wire::DataQualityFindingStatus::Acknowledged) => Ok(PartyFindingStatus::Acknowledged),
        Ok(wire::DataQualityFindingStatus::Waived) => Ok(PartyFindingStatus::Waived),
        Ok(wire::DataQualityFindingStatus::Remediated) => Ok(PartyFindingStatus::Remediated),
        Ok(wire::DataQualityFindingStatus::Unspecified) | Err(_) => Err(SdkError::invalid_argument("data_quality.finding.status", "Finding status filter is invalid")),
    }).transpose()
}

fn validated_severity_filter(value: Option<i32>) -> Result<Option<QualitySeverity>, SdkError> {
    value.map(|value| match wire::QualitySeverity::try_from(value) {
        ...
        Ok(wire::QualitySeverity::Unspecified) | Err(_) => Err(SdkError::invalid_argument("data_quality.finding.severity", "Finding severity filter is invalid")),
    }).transpose()
}
...
fn status_filter_wire(value: Option<PartyFindingStatus>) -> i32 { value.map_or(0, status_to_wire) }
fn severity_filter_wire(value: Option<QualitySeverity>) -> i32 { value.map_or(0, severity_to_wire) }

fn finding_cursor_binding(request: &QueryRequest, filter_hash: [u8; 32], page_size: u32) -> Result<CursorBinding, SdkError> {
    Ok(CursorBinding {
...
fn decode_finding_after(adapter: &DataQualityQueryAdapter, token: &str, binding: &CursorBinding) -> Result<Option<RecordQueryContinuation>, SdkError> {
    if token.is_empty() { return Ok(None); }
    let continuation = adapter.cursor_codec.decode(token, binding).map_err(cursor_error)?;
    let sort_value = String::from_utf8(continuation.sort_key).map_err(|_| cursor_invalid())?;
    let after = RecordQueryContinuation { sort_value, record_id: continuation.record_id };
    after.validate()?;
    Ok(Some(after))
}

fn encode_finding_next(adapter: &DataQualityQueryAdapter, binding: &CursorBinding, next: Option<&RecordQueryContinuation>) -> Result<String, SdkError> {
    next.map(|next| adapter.cursor_codec.encode(binding, &CursorContinuation {
        sort_key: next.sort_value.as_bytes().to_vec(),
        record_id: next.record_id.clone(),
    }).map_err(cursor_error)).transpose().map(|value| value.unwrap_or_default())
}

fn process_cursor_key() -> [u8; 32] {
    ...
    let mut key = [0_u8; 32];
    let seed = format!("{}:{}:{:?}", std::process::id(), std::thread::current().name().unwrap_or("unnamed"), std::time::SystemTime::now());
