...
fn finding_list_contract_binds_filters_page_size_and_signed_cursor() {
    ...
    assert_eq!(
        data_quality::ListDataQualityFindingsByPartyRequest::decode(bytes.as_slice()).unwrap(),
        request
    );
}
...
fn completeness_result_contract_preserves_exact_component_lineage() {
    let result = data_quality::PartyCompletenessResult {
        completeness_result_ref: Some(data_quality::PartyCompletenessResultRef {
            completeness_result_id: "dq-completeness-result-1".to_owned(),
        }),
        ...
        completeness_profile_version_ref: Some(data_quality::PartyCompletenessProfileVersionRef {
            completeness_profile_version_id: "dq-profile-1".to_owned(),
        }),
        ...
        components: vec![data_quality::PartyCompletenessComponentResult {
            ...
            rule_outcome_ref: Some(data_quality::PartyRuleOutcomeRef {
                rule_outcome_id: "dq-rule-outcome-1".to_owned(),
            }),
    ...
    assert_eq!(
        data_quality::PartyCompletenessResult::decode(bytes.as_slice()).unwrap(),
        result
    );
}
...
fn read_descriptor_identities_are_stable_and_distinct() {
    ...
    assert_eq!(
        finding,
        message_descriptor_hash("crm.data_quality.v1.GetDataQualityFindingRequest")
    );
