...
impl ResourceFamily {
    pub(crate) fn token(self) -> &'static str {
        match self {
            Self::EvaluationJob => "job",
            Self::EvaluationInput => "input",
            Self::RuleOutcome => "outcome",
            Self::Finding => "finding",
            Self::FindingObservation => "observation",
            Self::CompletenessResult => "completeness",
            Self::RemediationAttempt => "remediation",
        }
    }

    fn parse(value: &str) -> Result<Self, SdkError> {
        match value {
            "job" => Ok(Self::EvaluationJob),
            "input" => Ok(Self::EvaluationInput),
            "outcome" => Ok(Self::RuleOutcome),
            "finding" => Ok(Self::Finding),
            "observation" => Ok(Self::FindingObservation),
            "completeness" => Ok(Self::CompletenessResult),
            "remediation" => Ok(Self::RemediationAttempt),
...
pub(crate) fn validate_request_contract(request: &QueryRequest) -> Result<(), SdkError> {
    let definition = data_quality_privacy_scope_definition()?;
...
pub(crate) fn validate_wire_request(
    ...
    let (page_number, cursor_state) = if contribution.cursor.is_empty() {
        (
            1,
            CursorState {
                family: ResourceFamily::EvaluationJob,
...
fn decode_cursor(
    ...
    if family == ResourceFamily::EvaluationJob && after_record_id.is_none() {
        return Err(cursor_invalid());
    }
...
fn cursor_digest(
    ...
    framed_digest(
        b"crm.data-quality.privacy.scope.request-cursor/v1",
        &[
...
fn cursor_invalid() -> SdkError {
    invalid_contract(
        "DATA_QUALITY_PRIVACY_SCOPE_CURSOR_INVALID",
        "The Data Quality privacy scope cursor is invalid.",
    )
}
